...
plot_seed_a_vs_b.py
===================
Seed A (PAC-dominant) vs Seed B (balanced) comparison figure.

Loads two Pareto seeds from pareto_seeds_fresh_DE.json and generates two figures:
  Figure 1 — Panel A: 30s timeseries (cortex r_E + thalamic r_TCR) with
               detected UP peaks (red ▼) and spindle peaks (green ▲)
             Panel B: Event-locked spindle envelope around UP peaks
               (grey traces = individual events, red bold = mean)
  Figure 2 — Panel C: Polar PAC histogram (18 bins)
             Panel D: Quantitative metrics table comparing Seed A and Seed B

This figure provides evidence for "why SBI rejected A and accepted B":
Seed A has PAC_compound=1.0 (MI≈0.137, dominant PAC) vs
Seed B balanced (MI≈0.07, score=0.674 > Seed A 0.510).

USAGE
-----
    python plot_scripts/plot_seed_a_vs_b.py \
        --pareto-json S4_v7_repair/pareto_seeds_fresh_DE.json \
        --seed-a-label A \
        --seed-b-label B \
        --out outputs/fig_seed_a_vs_b.png \
        --sim-dur-ms 60000

Source
------
Adapted from plot_fig7_compare_v7_vs_v8.py (90% shared code).
Pareto JSON: seeds[*].{tag, params (8-dim), objectives.score, pac_metrics.MI}
"""
...
def load_seed_from_pareto(path, seed_label):
    """
    Load one seed from pareto_seeds_fresh_DE.json by tag label ('A', 'B', 'C').

    JSON structure: seeds[*].{tag, params (8-dim full set), objectives.score, pac_metrics.MI}
    All 8 params are present in the JSON — no supplement from other files needed.

    Returns dict with 8 parameter keys + '_score' (from objectives.score) at top level.
    """
    with open(path, "r", encoding="utf-8") as f:
        raw = json.load(f)

    for seed in raw["seeds"]:
        if seed["tag"] == seed_label:
            bp = dict(seed["params"])
            bp["_score"] = seed["objectives"].get("score")
            bp["_pac_mi"] = seed.get("pac_metrics", {}).get("MI")
            bp["_pac_compound"] = seed["objectives"].get("PAC_compound")
            required = ["mue", "mui", "b", "tauA", "g_LK", "g_h", "c_th2ctx", "c_ctx2th"]
            missing = [k for k in required if k not in bp]
            if missing:
                raise ValueError(f"Seed {seed_label}: missing parameters {missing} in {path}")
            return bp

    raise KeyError(f"Seed label '{seed_label}' not found in {path}. "
                   f"Available: {[s['tag'] for s in raw['seeds']]}")
...
def plot_panel_D(ax, m_a, m_b, n_evts_a, n_evts_b):
    """Panel D: comparison table as a styled matplotlib table."""
    ax.axis('off')

    score_a = m_a.get('_score')
    score_b = m_b.get('_score')
    if score_a is not None and score_b is not None and score_a > 0:
        score_delta = f"{(score_b / score_a - 1) * 100:+.1f}%"
    else:
        score_delta = "—"

    rows = [
        ("Score (overall)",
            f"{score_a:.4f}" if score_a is not None else "—",
            f"{score_b:.4f}" if score_b is not None else "—",
            score_delta),
        ("phase argmax",
            f"{m_a['phase_argmax_deg']:+.0f}°",
            f"{m_b['phase_argmax_deg']:+.0f}°",
            "stable"),
        ("phase concentration",
            f"{m_a['concentration']:.3f}",
            f"{m_b['concentration']:.3f}",
            f"{(m_b['concentration']/m_a['concentration']-1)*100:+.0f}%"),
        ("up_down_ratio",
            f"{m_a['up_down_ratio']:.2f}",
            f"{m_b['up_down_ratio']:.2f}",
            f"{(m_b['up_down_ratio']/m_a['up_down_ratio']-1)*100:+.0f}%"),
        ("MI",
            f"{m_a['mi']:.4f}",
            f"{m_b['mi']:.4f}",
            f"{(m_b['mi']/m_a['mi']-1)*100:+.0f}%"),
        ("# spindle events",
            f"{n_evts_a}",
            f"{n_evts_b}",
            f"{n_evts_b - n_evts_a:+d}"),
        ("# SO cycles",
            f"{m_a['n_so_cycles']}",
            f"{m_b['n_so_cycles']}",
            f"{m_b['n_so_cycles'] - m_a['n_so_cycles']:+d}"),
    ]

    headers = ["Metric", "Seed A", "Seed B", "Δ"]
...
def main():
    ...
    parser.add_argument("--pareto-json", type=str,
                        default="S4_v7_repair/pareto_seeds_fresh_DE.json")
    parser.add_argument("--seed-a-label", type=str, default="A",
                        help="Tag of the first seed (PAC-dominant). Default: A")
    parser.add_argument("--seed-b-label", type=str, default="B",
                        help="Tag of the second seed (balanced). Default: B")
    parser.add_argument("--out", type=str,
                        default="outputs/fig_seed_a_vs_b.png")
    ...
    print("=" * 60)
    print("Seed A vs Seed B PAC comparison figure")
    print("=" * 60)

    bp_a = load_seed_from_pareto(args.pareto_json, args.seed_a_label)
    bp_b = load_seed_from_pareto(args.pareto_json, args.seed_b_label)

    print(f"\nSeed A ({args.seed_a_label}): score={bp_a['_score']:.5f}, "
          f"MI={bp_a.get('_pac_mi', '?')}, PAC_compound={bp_a.get('_pac_compound', '?'):.4f}")
    print(f"Seed B ({args.seed_b_label}): score={bp_b['_score']:.5f}, "
          f"MI={bp_b.get('_pac_mi', '?')}, PAC_compound={bp_b.get('_pac_compound', '?'):.4f}")

    res_a = analyze_one(bp_a, args.sim_dur_ms, "Seed A (PAC-dominant)")
    res_b = analyze_one(bp_b, args.sim_dur_ms, "Seed B (balanced)")

    # Derive output paths from --out stem
    out_base = Path(args.out)
    out_base.parent.mkdir(parents=True, exist_ok=True)
    stem = out_base.stem          # e.g. "fig_seed_a_vs_b"
    ...
    # Score strings for titles
    score_a_str = f"{bp_a['_score']:.5f}" if bp_a.get('_score') is not None else "?"
    score_b_str = f"{bp_b['_score']:.5f}" if bp_b.get('_score') is not None else "?"
    ...
    fig1.suptitle("Fig. 7 — SO-Spindle comparison: Seed A vs Seed B  |  Timeseries & Event-locked Envelope",
                  fontsize=13, fontweight='bold', y=0.975)

    # Panel A — Seed A (left half, row 0): two stacked subaxes
    sg_a = gs1[0, 0:2].subgridspec(2, 1, hspace=0.06)
    ax_A_a_top = fig1.add_subplot(sg_a[0])
    ax_A_a_bot = fig1.add_subplot(sg_a[1])
    plot_panel_A((ax_A_a_top, ax_A_a_bot),
                  res_a["r_ctx"], res_a["r_thal"],
                  res_a["ctx_peaks"], res_a["sp_peaks"], FS_SIM,
                  title_prefix=f"Seed A — log_prob={score_a_str}")

    # Panel A — Seed B (right half, row 0): two stacked subaxes
    sg_b = gs1[0, 2:4].subgridspec(2, 1, hspace=0.06)
    ax_A_b_top = fig1.add_subplot(sg_b[0])
    ax_A_b_bot = fig1.add_subplot(sg_b[1])
    plot_panel_A((ax_A_b_top, ax_A_b_bot),
                 res_b["r_ctx"], res_b["r_thal"],
                 res_b["ctx_peaks"], res_b["sp_peaks"], FS_SIM,
                 title_prefix=f"Seed B — log_prob={score_b_str}")

    # Panel B — event-locked spindle envelope (row 1)
    ax_B_a = fig1.add_subplot(gs1[1, 0:2])
    plot_panel_B(ax_B_a, res_a["r_thal"], res_a["ctx_peaks"], FS_SIM,
                 title="Seed A: event-locked spindle envelope")
    ax_B_b = fig1.add_subplot(gs1[1, 2:4])
    plot_panel_B(ax_B_b, res_b["r_thal"], res_b["ctx_peaks"], FS_SIM,
                 title="Seed B: event-locked spindle envelope")

    # Share y-axis between B panels
    ymax_B = max(ax_B_a.get_ylim()[1], ax_B_b.get_ylim()[1])
    ax_B_a.set_ylim([0, ymax_B])
    ax_B_b.set_ylim([0, ymax_B])
    ...
    fig2.suptitle("Fig. 7 — SO-Spindle PAC: Seed A vs Seed B  |  Polar Histograms & Quantitative Metrics",
                  fontsize=13, fontweight='bold', y=0.975)

    # Panel C — polar PAC histograms (row 0)
    ax_C_a = fig2.add_subplot(gs2[0, 0:2], projection='polar')
    plot_panel_C(ax_C_a, res_a["bin_centers"], res_a["mean_amp"],
                 title="Seed A: polar PAC histogram (18 bins)")
    ax_C_b = fig2.add_subplot(gs2[0, 2:4], projection='polar')
    plot_panel_C(ax_C_b, res_b["bin_centers"], res_b["mean_amp"],
                 title="Seed B: polar PAC histogram (18 bins)")

    # Share radial axis
    rmax = max(res_a["mean_amp"].max(), res_b["mean_amp"].max()) * 1.1
    ax_C_a.set_ylim([0, rmax])
    ax_C_b.set_ylim([0, rmax])

    # Panel D — comparison table (row 1, full width)
    ax_D = fig2.add_subplot(gs2[1, :])
    plot_panel_D(ax_D, res_a["metrics"], res_b["metrics"],
                 len(res_a["sp_peaks"]), len(res_b["sp_peaks"]))
    ax_D.set_title("Seed A vs Seed B: quantitative metric comparison",
