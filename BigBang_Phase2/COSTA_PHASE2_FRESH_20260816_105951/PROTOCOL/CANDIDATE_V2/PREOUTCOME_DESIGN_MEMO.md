# COSTA Phase 2 Candidate V2 — Pre-outcome Design Memo

Campaign: `COSTA_PHASE2_FRESH_20260816_105951`  
Protocol candidate: `COSTA_PHASE2_PROTOCOL_CANDIDATE_V2`  
...
Candidate V2 specifies a self-contained, current-campaign thalamocortical diagnostic bench and an actual T1–T8 Phase 2A execution protocol, but it does not bind that bench to the unidentified R4 executable. The clean categorical substrate therefore remains exactly `M1/C0/P0`; the bench can yield at most non-transferable synthetic diagnostic evidence and cannot assign formal C1 or C2. Formal Phase 2B is prospectively fixed as `NOT_ESTIMABLE` before outcomes because the clean data audit certifies zero eligible public/development Night-1 personalized models, zero clean executable personalized-model artifacts, and no exact R4 execution identity. The minimum formal denominator is three subjects, and synthetic draws cannot replace them.

Candidate V2 is a protocol freeze candidate only. The unchanged dynamical model retains bench identity `PHASE2_CONTROL_BENCH_V1`; its executable protocol/claim route version is `2.0.0`, and the package identity is `CANDIDATE_V2`. This role did not freeze it, release implementation, inspect code, or execute science. Root and an independent auditor retain those authorities.

## Exact V1-to-V2 change log

V2 preserves V1 equations, parameter defaults/bounds, uncertainty draws, timebase, target identities/priorities, non-phase intervention grids, observables, practical margins, subject gate, advancement ceiling, C2 coverage, and `M1/C0/P0` claim boundary except for the five changes strictly required by the immutable audit.

| Audit blocker | Exact V2 repair | Propagated artifacts |
|---|---|---|
| `PF-B001` | Replaced the underdefined T6/T7 phase trigger with a 200 Hz causal one-pole HP/LP scheduler using fixed coefficients, zero states, 30 s warmup, strict upward crossings, three-interval median period clamped to 134–400 samples, forward modulo phase delays, round-half-up quantization, one-sample minimum latency, unique `-pi` endpoint, bounded queue/overlap behavior, exact 500-step packet samples, and exhaustive decision logs. No delay calibration or subtraction exists. | `mechanistic_parameter_map.json`, `phase2_protocol.json`, `control_target_registry.json`, `claim_registry.json` |
| `PF-B002` | Replaced impossible six 30 s T8 windows with six adjacent 25 s windows at `[15,40)`, `[40,65)`, `[65,90)`, `[90,115)`, `[115,140)`, and `[140,165)` s, entirely inside retained `[10,170)` s. Each uses exact 200 Hz Hann Welch parameters `nperseg=5000`, `noverlap=0`, `nfft=10000`; filters and sham thresholds are global, and events require strict window containment. | `mechanistic_parameter_map.json`, `phase2_protocol.json`, `control_target_registry.json`, `claim_registry.json` |
| `PF-B003` | Added executed `-pi/2` and `+pi/2` Level-A conditions, removed duplicate-equivalent `+pi`, and froze the canonical unique grid `[-pi,-2pi/3,-pi/2,-pi/3,0,pi/3,pi/2,2pi/3]`. Unique conditions per seed-draw rise from 49 to 50 and maximum Level-A runs from 1,764 to 1,800. | `mechanistic_parameter_map.json`, `phase2_protocol.json`, `control_target_registry.json`, `claim_registry.json` |
| `PF-B004` | Added a fully implementable studentized max-absolute-deviation bootstrap: 10,000 joint hierarchical paired resamples, explicit orientation and circular wrapping, fixed scale floor, 9501st order-statistic quantile, and distinct seeded families for all target-decision components (`860218`), all selected Level-B cells (`860220`), all 64 off-target cells (`860221`), and future C2 interactions (`860222`). Missing cells never become zero; Holm remains a separate hypothesis-test rule. | all six JSON artifacts and this memo |
| `PF-B005` | Added exact T4 paired event attribution at `d=+1.5`: earliest RMS-peak times, one-to-one greedy matching within 50 samples with complete tie order, unmatched ADDED/LOST events, deterministic packet association, a full post-offset 11–16 Hz cycle test, `N_added>=30` from at least 10 blocks, simultaneous `F_auto>0.50`, and explicit PASS/D/F/NOT_ESTIMABLE behavior. | `mechanistic_parameter_map.json`, `phase2_protocol.json`, `control_target_registry.json`, `claim_registry.json` |

No change was motivated by a scientific outcome; none was visible.
...
| `PROTOCOL/PROTOCOL_FREEZE_V1.json` | `1A27094D05DD24848E7F8D9643936D69078464B864EFA18621E022A04410F3F0` |
| `GOVERNANCE/PROTOCOL_FREEZE_AUDIT.json` | `053ED3636CD1F40597524DB2C5369F87B5FC52696EB62C12895917BF9749BE14` |

The sanitized R4 artifact was used only for its permitted value-free categorical and integrity role: it establishes the historical categorical substrate and the absence of an executable identity. Its values did not determine the bench equations, thresholds, grids, target ranking, or scientific interpretation. The repository capability inventory was used only to avoid silently claiming that an existing canonical simulator, detector, or controller had been released. All numeric design choices in Candidate V2 are new, prospective bench conventions.
...
Every actionable channel is named **GENERIC EXTERNAL FORCING**. Candidate V2 includes bounded additive cortical DC, cortical slow-frequency sinusoidal forcing, cortical pulse forcing, relay-versus-reticular sigma packets, continuous relay-versus-reticular sigma forcing, causal phase-triggered thalamic packets, and balanced cortical/thalamic versus inhibitory/reticular DC forcing. Injection population, timing, amplitude semantics, units, and ranges are frozen.
...
| Exploratory | T8 | A multimetric model-regime construct with the greatest proxy and classification dependence; it cannot advance under V2. |
