"""
s4_personalize_fig7_v8.py
=========================
Physics-constrained thalamocortical personalisation -- V8a.

V8a = V7 + cortical spindle observability audit patch.

V7 closed several event-level loopholes by verifying thalamic spindle
events through T8/T12, but the EEG-analog signal in this model is the
cortical node. The V7 selected point can therefore pass the event-level
audit while producing zero cortical spindle events. V8a keeps the V7
physiology checks and adds T13, a permissive nonzero cortical spindle
observability gate measured on r_ctx.

This is an audit patch, not proof of final model correctness.

Usage (from project root):
  python models/s4_personalize_fig7_v8.py
...
PARAMS_PATH  = f"data/patient_params_fig7_v8_{SUBJECT_ID}.json"
RECORDS_PATH = "outputs/evolution_fig7_v8_records.csv"
...
# V8a BOUNDS -- V7 with conservative c_th2ctx expansion for cortical observability
# ----------------------------------------------------------------
# Cortex (mue, mui, b, tauA): UNCHANGED from v5/v6.
#
# Thalamic (g_LK, g_h): UNCHANGED from v6 (sweep-calibrated, working).
#
# c_th2ctx: v7 [0.00, 0.05] -> v8a [0.00, 0.075].
...
BOUNDS = [
    ...
    (0.00,  0.075),                                            # c_th2ctx (V8a: conservative expansion)
...
# T13 constants (NEW in V8a): cortical spindle observability on r_ctx.
# T13 is deliberately permissive: it requires nonzero cortical spindle
# events and nonzero peak-verified cortical events, but does not require
# near-real EEG density as a hard constraint.
T13_CTX_EVT_MIN      = 1
...
def compute_constraints_v8(r_ctx, r_thal, f_c=None, p_c=None, fs=FS_SIM):
    """
    13 hard binary constraints for V8a. ALL must pass for feasibility.
    ...
    # T13 (V8a): cortical spindle observability on the EEG-analog r_ctx.
    # T8/T12 remain thalamic spindle reality checks; T13 only closes the
    # residual output-level loophole where cortical spindle events are zero.
    n_ctx_events = 0
    n_ctx_verified = 0
    mean_ctx_dur = 0.0
    ctx_density_per_min = 0.0
    ctx_verified_density_per_min = 0.0
    try:
        sos_ctx = butter(4, [SPINDLE_LO, SPINDLE_HI],
                         btype="band", fs=fs, output="sos")
        ctx_filtered = sosfiltfilt(sos_ctx, r_ctx)
        ctx_envelope = np.abs(hilbert(ctx_filtered))
        sigma_samples = SPINDLE_ENV_SMOOTH_MS * fs / 1000.0
        ctx_env_smooth = gaussian_filter1d(ctx_envelope, sigma=sigma_samples)
        ctx_thresh = np.percentile(ctx_env_smooth, SPINDLE_EVT_PCTILE)
        ctx_above = (ctx_env_smooth > ctx_thresh).astype(np.int8)
        ctx_diff = np.diff(np.concatenate(([0], ctx_above, [0])))
        ctx_starts = np.where(ctx_diff == 1)[0]
        ctx_ends = np.where(ctx_diff == -1)[0]
        ctx_durations = (ctx_ends - ctx_starts) / fs
        ctx_valid = ((ctx_durations >= T13_CTX_DUR_LO_S) &
                     (ctx_durations <= T13_CTX_DUR_HI_S))
        n_ctx_events = int(ctx_valid.sum())
        if n_ctx_events > 0:
            mean_ctx_dur = float(ctx_durations[ctx_valid].mean())
            ctx_starts_valid = ctx_starts[ctx_valid]
            ctx_ends_valid = ctx_ends[ctx_valid]
            for s, e in zip(ctx_starts_valid, ctx_ends_valid):
                event = r_ctx[s:e]
                ...
                try:
                    ...
                    if (p_ev[sp_m].max() / ns_peak) > T13_PEAK_INSIDE_RATIO:
                        n_ctx_verified += 1
                except Exception:
                    continue
        duration_min = len(r_ctx) / fs / 60.0
        if duration_min > 0:
            ctx_density_per_min = n_ctx_events / duration_min
            ctx_verified_density_per_min = n_ctx_verified / duration_min
    except Exception:
        n_ctx_events = 0
        n_ctx_verified = 0
        mean_ctx_dur = 0.0
    ...
    t13 = ((n_ctx_events >= T13_CTX_EVT_MIN) and
           (n_ctx_verified >= T13_CTX_VERIFIED_MIN))
    details["T13"] = t13
    details["T13_n_ctx_events"] = n_ctx_events
    details["T13_n_ctx_verified"] = n_ctx_verified
    details["T13_ctx_density_per_min"] = round(ctx_density_per_min, 3)
    details["T13_ctx_verified_density_per_min"] = round(ctx_verified_density_per_min, 3)
    details["T13_mean_ctx_dur"] = round(mean_ctx_dur, 3)
...
def compute_feasibility_score(con):
    ...
    # T13 (NEW in V8a): want nonzero cortical events and verification.
    if con["T13"]:
        scores.append(1.0)
    else:
        evt_score = con.get("T13_n_ctx_events", 0) / T13_CTX_EVT_MIN
        ver_score = con.get("T13_n_ctx_verified", 0) / T13_CTX_VERIFIED_MIN
        scores.append(np.clip(min(evt_score, ver_score), 0, 0.99))
...
def compute_fitness_v8(params_vec,
                       target_psd, target_freqs,
                       target_periodic, fooof_freqs):
    """
    V8a fitness: 13 constraints + thalamic and cortical spindle rewards.

    if ALL 13 constraints pass (feasible):
        fitness = 0.40*shape_r + 0.20*so_power
                  + 0.15*thal_spindle_power + 0.25*ctx_spindle_power
        where:
          so_power           = clip((T4_q - 1) / 4, 0, 1)
                             (V7 CHANGE #3: T4_q-derived, FOOOF-free)
          thal_spindle_power = clip(T12_n_verified / 15, 0, 1)
          ctx_spindle_power  = clip(T13_ctx_verified_density_per_min / 2, 0, 1)
                             (V7 CHANGE #2: event-count-based, FOOOF-free)
        Range: [0, ~1].

    else:
        fitness = -10 + 10 * mean(constraint_scores)  [range: -10 to ~-0.1]
        (gives DE gradient signal on infeasible solutions)
    ...
    # ══════════════════════════════════════════════════════════════════
    n_passed, con = compute_constraints_v8(
    ...
    if con["feasible"]:
        ...
        thal_spindle_power = float(np.clip(n_ver / 15.0, 0.0, 1.0))
        spindle_power = thal_spindle_power  # V7-compatible alias

        ctx_density = con.get("T13_ctx_verified_density_per_min", 0.0)
        ctx_spindle_power = float(np.clip(ctx_density / 2.0, 0.0, 1.0))

        # V8a fitness adds cortical spindle observability reward.
        fitness = (W_SHAPE * shape_r
                   + W_SO * so_power
                   + W_THAL_SPINDLE * thal_spindle_power
                   + W_CTX_SPINDLE * ctx_spindle_power)
    else:
        ...
        fitness = -10.0 + 10.0 * (sum(c_scores) / 13.0)
    ...
    record.update({
        ...
        "thal_spindle_power": round(thal_spindle_power, 6),
        "ctx_spindle_power":  round(ctx_spindle_power, 6),
        ...
        "T13":            int(con["T13"]),
        "T13_n_ctx_events": con["T13_n_ctx_events"],
        "T13_n_ctx_verified": con["T13_n_ctx_verified"],
        "T13_ctx_density_per_min": con["T13_ctx_density_per_min"],
        "T13_ctx_verified_density_per_min": con["T13_ctx_verified_density_per_min"],
        "T13_mean_ctx_dur": con["T13_mean_ctx_dur"],
        "eval":           _eval_count,
    })
...
def _callback(xk, convergence):
    ...
    print(
        ...
        f"  ctxsp={bp.get('ctx_spindle_power',0):.3f}"
        f"  MI={bp.get('T9_mi',0):.4f}"
        f"  phi={bp.get('T10_phase',0):+.2f}"
        f"  lag={bp.get('T11_lag_ms',0):+.0f}ms"
        f"  nver={bp.get('T12_n_verified',0)}"
        f"  ctxver={bp.get('T13_n_ctx_verified',0)}"
        f"  ctxdens={bp.get('T13_ctx_verified_density_per_min',0):.2f}/min"
        f"  evals={_eval_count}"
        f"  elapsed={elapsed:.0f}s"
    )
    return False
...
def main():
    ...
    print("=" * 70)
    print("V8a Evolution: V7 + cortical spindle observability audit patch")
    print("               T13 is a permissive nonzero gate on cortical r_ctx")
    print("  FOOOF=" + ("ON" if HAS_FOOOF else "OFF (fallback)") +
          "  (used for shape_r only; spindle rewards are event-based)")
    print(f"  8 params, 13 constraints (continuous satisfaction)")
    print(f"  SIM_DUR_MS={SIM_DUR_MS/1000:.0f}s "
          f"(inherits V7 60s simulation)")
    print(f"  Bounds:")
    ...
    print(f"    c_th2ctx [{BOUNDS[6][0]:.3f}, {BOUNDS[6][1]:.3f}]  "
          f"(V8a conservative expansion from V7 [0.00, 0.05])")
    ...
    print(f"  T13 cortical spindle observability: "
          f"events >= {T13_CTX_EVT_MIN}, verified >= {T13_CTX_VERIFIED_MIN}, "
          f"dur {T13_CTX_DUR_LO_S}-{T13_CTX_DUR_HI_S}s "
          f"(density recorded, not hard-constrained)")
    ...
    print(f"    thal_spindle_power = clip(T12_n_verified/15, 0, 1)")
    print(f"    ctx_spindle_power  = clip(T13_ctx_verified_density_per_min/2, 0, 1)")
    print(f"    total = {W_SHAPE:.2f}*shape_r + "
          f"{W_SO:.2f}*so_power + "
          f"{W_THAL_SPINDLE:.2f}*thal_spindle_power + "
          f"{W_CTX_SPINDLE:.2f}*ctx_spindle_power")
    print(f"  Infeasible scoring: continuous [-10, ~-0.1] (scaled for 13 C's)")
    print(f"  popsize={DE_POPSIZE} x {len(PARAM_NAMES)} = "
          f"{DE_POPSIZE*len(PARAM_NAMES)} individuals/gen  (V8a keeps V7 "
    ...
    print(f"\n{'='*55}")
    print("V8a Validation")
    print(f"{'='*55}")
    print(f"  score         = {best.get('score'):.4f}")
    print(f"  feasible      = {best.get('feasible')}")
    print(f"  n_passed      = {best.get('n_passed')}/13")
    print(f"  shape_r       = {best.get('shape_r'):.4f}")
    print(f"  so_power      = {best.get('so_power'):.4f}")
    print(f"  thal_spindle_power = {best.get('thal_spindle_power'):.4f}")
    print(f"  ctx_spindle_power  = {best.get('ctx_spindle_power'):.4f}")
    print(f"")
    print(f"  Constraints (T1-T12 from V7, T13 cortical observability new in V8a):")
    ...
    print(f"    T13 Cortical spindle:   {'PASS' if best.get('T13') else 'FAIL'}"
          f"  [events={best.get('T13_n_ctx_events')}, "
          f"verified={best.get('T13_n_ctx_verified')}, "
          f"density={best.get('T13_ctx_verified_density_per_min')}/min, "
          f"mean_dur={best.get('T13_mean_ctx_dur')}s]")
    print(f"")
    print(f"  Best parameters:")
    for k in PARAM_NAMES:
        print(f"    {k}: {best.get(k):.4f}")
    print(f"\nNext: run only approved static/smoke checks before any DE or 300s validation.")
