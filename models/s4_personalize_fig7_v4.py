"""
s4_personalize_fig7_v4.py
=========================
Physics-constrained thalamocortical personalisation — Version 4.

V4 key changes from V3:
  - Constraints and rewards are SEPARATED (Deb feasibility rules).
  - Removed: spindle_gate, so_overshoot_penalty, narrow_spindle_penalty,
    and all associated hyperparameters (26 constants → 12).
  - T4 upgraded to T4+: SO peak must have Q-factor > threshold (not just
    correct frequency). Rejects excitable-regime solutions with flat PSD.
  - NEW T6: SO regularity — inter-burst interval CV must be < 0.4.
    Rejects noise-driven sparse pulses, requires limit-cycle oscillation.
  - NEW T7: Spindle burstiness — envelope CV must be > 0.7.
    Rejects continuous thalamic oscillation, requires intermittent bursts.
  - Feasible solutions ranked by V1-style simple reward:
    fitness = 0.50*shape_r + 0.25*so_power + 0.25*spindle_power
  - Infeasible solutions ranked by n_constraints_passed (always below feasible).
  - PAC-MVL computed as post-optimization validation metric (not in fitness).

Usage (from project root):
  python models/s4_personalize_fig7_v4.py
...
PARAMS_PATH  = f"data/patient_params_fig7_v4_{SUBJECT_ID}.json"
RECORDS_PATH = "outputs/evolution_fig7_v4_records.csv"
...
def compute_fitness_v4(params_vec,
    ...
    """
    V4 fitness: hard constraints (Deb rules) + simple rewards.

    if ALL 7 constraints pass:
        fitness = 0.50*shape_r + 0.25*so_power + 0.25*spindle_power
    else:
        fitness = -1e6 + n_constraints_passed   (always below any feasible)
    """
    ...
    if con["feasible"]:
        ...
        # Deb rule: infeasible solutions ranked by n_passed, always below feasible
        fitness = -1e6 + n_passed
...
def main():
    ...
    print("=" * 65)
    print("V4 Evolution: Deb-constrained N3 fitting")
    print("  FOOOF=" + ("ON" if HAS_FOOOF else "OFF (fallback)"))
    print(f"  8 params, 7 hard constraints (Deb feasibility rules)")
    print(f"  NEW: T4+ SO Q-factor > {SO_Q_MIN}")
    print(f"  NEW: T6 IBI CV < {IBI_CV_MAX} (SO regularity)")
    print(f"  NEW: T7 spindle envelope CV > {SPINDLE_CV_MIN} (burstiness)")
    ...
    print(f"\nNext: run python plot_scripts/plot_fig7_v4_fast.py")
