...
Usage (from project root):
  python models/s4_personalize_fig7_v4_improve_v2.py
...
SPINDLE_EVT_MIN  = 5         # T8: min number of spindle events in 30s (NEW)
SPINDLE_DUR_LO_S = 0.3       # T8: min spindle event duration [s] (NEW)
SPINDLE_DUR_HI_S = 2.0       # T8: max spindle event duration [s] (NEW)
SPINDLE_EVT_THRESH_FACTOR = 1.5  # T8: envelope threshold = mean + factor*std (NEW)
...
def compute_constraints_v4(r_ctx, r_thal, f_c=None, p_c=None, fs=FS_SIM):
    """
    8 hard binary constraints for V4-improve.  ALL must pass for feasibility.
    ...
    T8:        NEW — enough discrete spindle events with valid duration
    """
    details = {}
...
def compute_fitness_v4(params_vec,
    ...
    if ALL 8 constraints pass:
        fitness = 0.50*shape_r + 0.25*so_power + 0.25*spindle_power  [range: 0 to ~1]
    else:
        fitness = -10 + 8 * mean(constraint_scores)  [range: -10 to -2]
    ...
    if con["feasible"]:
        ...
        fitness = -10.0 + 8.0 * (sum(c_scores) / 8.0)
...
def main():
    ...
    print(f"  8 params, 8 constraints (continuous satisfaction, not -1M cliff)")
    ...
    print(f"  T8 spindle events >= {SPINDLE_EVT_MIN}, dur {SPINDLE_DUR_LO_S}-{SPINDLE_DUR_HI_S}s (NEW)")
    ...
    print(f"  n_passed      = {best.get('n_passed')}/8")
